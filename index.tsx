...
const ProjectsSection: React.FC<{ setRef: (el: HTMLElement | null) => void }> = React.memo(({ setRef }) => {
    const projects = [
        { title: "Secure File Transfer Protocol", description: "A C++ application ensuring encrypted file transfers using modern cryptographic libraries.", technologies: ["C++", "OpenSSL", "Socket Programming"], link: "https://github.com/samkv2/SFTP-CPP" },
        { title: "Intrusion Detection System (PoC)", description: "A Java-based proof-of-concept IDS analyzing network packets for suspicious patterns.", technologies: ["Java", "Pcap4j", "Rule Engines"], link: "https://github.com/samkv2/IDS-PoC" },
...
interface Certificate {
    ...
    image?: string;
    delay?: string;
    onImageClick?: (imgSrc: string, rect: DOMRect) => void;
}
...
const CertificateCard: React.FC<Certificate> = React.memo(({ title, issuer, date, icon, image, delay, onImageClick }) => {
    ...
    return (
        <div ref={ref} className={`certificate-card`} >
            ...
            <div className="card-content-wrapper" style={{ width: '100%', padding: '0 5px' }}>
...
const Lightbox: React.FC<{ isOpen: boolean; imageSrc: string | null; originRect: DOMRect | null; onClose: () => void }> = ({ isOpen, imageSrc, originRect, onClose }) => {
    const contentRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        if (isOpen) {
            // document.body.style.overflow = 'hidden'; // Removed as per user request to allow scrolling
            // Animation logic
            if (contentRef.current && originRect) {
                const el = contentRef.current;
                const { left, top, width, height } = originRect;
                const centerX = left + width / 2;
                const centerY = top + height / 2;

                // Since we want position fixed relative to viewport, we need to account for scroll if we were using absolute
                // But transparency + fixed overlay means we just need delta from center.
                // However, originRect is relative to viewport.

                // Window center
                const winCenterX = window.innerWidth / 2;
                const winCenterY = window.innerHeight / 2;

                const moveX = centerX - winCenterX;
                const moveY = centerY - winCenterY;
                ...
                // Final State
                el.style.transition = 'transform 0.5s cubic-bezier(0.2, 0.8, 0.2, 1), opacity 0.5s ease'; // Smooth pop
                el.style.transform = 'translate(0, 0) scale(1)';
                el.style.opacity = '1';
            }
        } else {
            // document.body.style.overflow = '';
            // Close animation (reverse) is handled by the "else" but we unmount/hide quickly in parent.
            // Check index.css or logic. 
            // We can implement exit animation logic here if we delay unmount, but parent controls isOpen.
            // Parent handles delay.
            if (contentRef.current && originRect) {
                const el = contentRef.current;
                const { left, top, width, height } = originRect;
                const centerX = left + width / 2;
                const centerY = top + height / 2;
                const moveX = centerX - window.innerWidth / 2;
                const moveY = centerY - window.innerHeight / 2;

                el.style.transition = 'transform 0.4s cubic-bezier(0.5, 0, 0.5, 1), opacity 0.4s ease'; // Magic lamp in
                el.style.transform = `translate(${moveX}px, ${moveY}px) scale(0)`;
                el.style.opacity = '0';
            }
        }
    ...
    return (
        <div className={`lightbox-overlay ${isOpen ? 'active' : ''}`} onClick={onClose}>
            <div className="lightbox-content" ref={contentRef} onClick={(e) => e.stopPropagation()}>
                ...
                <img src={imageSrc} alt="Certificate Zoomed" className="lightbox-image" />
            </div>
        </div>
    );
};
...
const CertificatesSection: React.FC<{ setRef: (el: HTMLElement | null) => void }> = React.memo(({ setRef }) => {
    ...
    const openLightbox = useCallback((imgSrc: string, rect: DOMRect) => {
    ...
    const closeLightbox = useCallback(() => {
        ...
        // Wait for animation to finish before clearing image
        setTimeout(() => {
        ...
        <SectionWrapper setRef={setRef} id="certificates" className="certificates-section" title="My Certifications">
            <div className="certificate-grid">
                {certificates.map((cert, index) => (
                    <CertificateCard key={cert.title} {...cert} delay={`${index * 0.1}s`} onImageClick={openLightbox} />
                ))}
            </div>
            <Lightbox isOpen={lightboxOpen} imageSrc={selectedImage} originRect={originRect} onClose={closeLightbox} />
        </SectionWrapper>
    );
});
